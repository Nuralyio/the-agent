...
export class FormFillingTest {
  ...
  async testSimpleFormFill(): Promise<void> {
    const instruction = "type 'John Doe' into the customer name field and type 'john@example.com' into the email address field";
    ...
    // Verify the action plan contains TYPE steps
  ...
  async testComprehensiveFormFill(): Promise<void> {
    ...
    // Test with a simpler instruction to avoid complex form logic
    const result = await executeTestInstruction(
      this.context.actionEngine,
      'Type "Jane Smith" in customer name field, type "555-123-4567" in phone field, type "jane@email.com" in email field',
      'Form Filling Test: Comprehensive Form Fill'
    );

    assert(result.success, 'Comprehensive form filling should complete successfully');

    // Verify multiple steps were generated for different field types
    assert(result.steps.length >= 3, 'Should generate multiple steps for comprehensive form');

    // Check for different action types (primarily TYPE actions for now)
  ...
  async testDynamicFormDiscovery(): Promise<void> {
    ...
    const result = await executeTestInstruction(
      this.context.actionEngine,
      'Fill in the customer name with "Test User" and email with "test@example.com"',
      'Form Filling Test: Dynamic Form Discovery'
    );

    assert(result.success, 'Dynamic form discovery should complete successfully');

    // Verify that the AI identified and used correct selectors for form fields
  ...
  async testFormFillWithValidation(): Promise<void> {
    const instruction = "type 'Jane Smith' into the customer name field, type 'jane@test.com' into the email field, and verify the fields are filled correctly";
  ...
  async testFormSubmission(): Promise<void> {
    const instruction = "fill the form with name 'Test User' and email 'test@domain.com', then submit it";
    ...
    // Verify the workflow contains filling and clicking
  ...
  async testMultiStepForm(): Promise<void> {
    // Navigate to a different form page first
  ...
  async testFormClearAndRefill(): Promise<void> {
    const instruction = "clear the form fields and then fill them with name 'Cleared User' and email 'cleared@example.com'";
    ...
    // The plan should contain multiple fill operations
    const fillSteps = result.steps.filter(step => step.step.type === 'fill');
  ...
  async testFullFormAutomation(): Promise<void> {
    ...
    const result = await executeTestInstruction(
      this.context.actionEngine,
      'Fill out the complete pizza order: customer name "Sarah Johnson", phone "555-987-6543", email "sarah@test.com", select large pizza size, select mushroom and onion toppings, set delivery time to "19:45", and add delivery instructions "Ring doorbell"',
      'Form Filling Test: Complete Form'
    );

    assert(result.success, 'Complete form automation should succeed');

    // Verify all types of actions were generated
    const typeSteps = result.steps.filter((step: any) => step.step.type === 'type');
    const clickSteps = result.steps.filter((step: any) => step.step.type === 'click');

    assert(typeSteps.length >= 4, 'Should have TYPE steps for text fields');
    assert(clickSteps.length >= 1, 'Should have CLICK steps for radio/checkboxes');
    assert(result.steps.length >= 6, 'Should generate comprehensive action plan');

    console.log(`✅ Full form automation completed with ${result.steps.length} steps`);
    console.log(`📊 TYPE actions: ${typeSteps.length}, CLICK actions: ${clickSteps.length}`);
  }
