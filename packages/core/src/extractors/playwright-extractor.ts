 ...
 */
export class PlaywrightContentExtractor implements ContentExtractor {
  ...
  async extractContent(options: ExtractionOptions = {}): Promise<ExtractedContent> {
    ...
    const result = await this.page.evaluate(
      (extractOptions) => {
        // The extraction function is defined inline here
        return (extractPageContent as any)(extractOptions);
      },
      opts
    );
  ...
  async extractVisibleText(options: ExtractionOptions = {}): Promise<string[]> {
    return this.page.evaluate((extractOptions) => {
      return (extractVisibleText as any)(extractOptions);
    }, options);
  }
   ...
   */
  private filterInteractiveElements(elements: ExtractedElement[]): ExtractedElement[] {
   ...
   */
  private filterHeadings(elements: ExtractedElement[]): ExtractedElement[] {
   ...
   */
  private filterLinks(elements: ExtractedElement[]): ExtractedElement[] {
   ...
   */
  private filterForms(elements: ExtractedElement[]): ExtractedElement[] {
